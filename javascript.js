    ...
    answersDiv.forEach((answer) => {
        answer.addEventListener("click", () => {
            ...
            if (!answer.classList.contains("checked")) {
                //removed checked from another answer
                answersDiv.forEach((answer) => {
                    answer.classList.remove("selected");
                });
                //add selected on currently clicked 
                answer.classList.add("selected");
                //after any answer selected enable submit btn
                submitBtn.disabled = false;

            }
        });
    });
    // after updating question restart timer
    time = timerPerQuestion.value;
    startTimer(time);
};
...
const checkAnswer = () => {
    ...
    if (selectedAnswer) {
        ...
        if (answer == questions[currentQuestion - 1].correct_answer) {
            ...
            //if answer incorrect   
            //add wrong class on selected but also shows correct answer

            //adding wrong on wrong
            selectedAnswer.classList.add("wrong");

            const currectAnswer = document.querySelectorAll(".answer").forEach((answer) => {
    ...
    //answer check done when time over
    //if nothing selected and time finish
    ...
    answerDiv.forEach((answer) => {
        ...
        //if checked remove hover effect
    });
...
//on click on next button show next question
